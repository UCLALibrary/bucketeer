...
The first way is to convert individual TIFF images to JPEG 2000 images on the local machine. It can receive individual requests, access the TIFF on a mounted directory, convert it into a JPEG 2000 image, and then upload it to S3. This method is triggered by a RESTful endpoint.

The second way is to convert TIFF images to JPEG 2000 images in batch. With this method, a CSV file is uploaded to Bucketeer, TIFF images from the CSV (on a locally mounted directory) are uploaded to an AWS S3 bucket, AWS Lambda picks up on that event and converts them into JPEG 2000 images, and then the AWS Lambda stores them in another S3 bucket. This method is triggered by uploading a CSV file through a Web page.
